...
- Updated motivation messages to use text-only format
- Fixed status message checks to use text-based detection (e.g., "successfully" instead of emoji)
- Created comprehensive Help Centre page with expandable FAQ sections
- Added Help link to sidebar navigation under Account section

**Previous Work (Nov 30, 2025)**:
**Authentication Flow Fixes**:
- Fixed dashboard redirect logic for unauthenticated users
- Fixed onboarding check to always verify via API (prevents redirect loop when JWT is stale)
- Simplified NextAuth cookie configuration for better cross-origin compatibility
- Cleaned up Express to Next.js routing for auth endpoints
...
- Completed "AI Mentor" to "AI Motivator" rebrand across 15+ files (pages, components, APIs, marketing copy)
